    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (v2f i) : SV_Target
            {
				...
				float2 topRightUV = i.uv + float2(_MainTex_TexelSize.x, _MainTex_TexelSize.y) * halfScaleCeiling;
				float2 bottomRightUV = i.uv + float2(_MainTex_TexelSize.x * halfScaleFloor, - _MainTex_TexelSize.y * halfScaleFloor);
				float2 topLeftUV = i.uv + float2( - _MainTex_TexelSize.x * halfScaleFloor, _MainTex_TexelSize.y * halfScaleFloor);
                ...
                edgeNormal = edgeDepth > _NormalThreshold ? 1 : 0;
                
                float edge = max(edgeDepth, edgeNormal);
                
                float4 edgeColour = float4(_Colour.rgb, _Colour.a * edge);
